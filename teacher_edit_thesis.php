...
// thesisEdit Sorgusu ----------------------------------------------
$colname_thesisEdit = "-1";
if (SAFE_GET('thesisID')) {
  $colname_thesisEdit = (get_magic_quotes_gpc()) ? SAFE_GET('thesisID') : addslashes(SAFE_GET('thesisID'));
}
$colname2_thesisEdit = "-1";
if (isset($row_loggeduser['teacherID'])) {
  $colname2_thesisEdit = (get_magic_quotes_gpc()) ? $row_loggeduser['teacherID'] : addslashes($row_loggeduser['teacherID']);
}
mysql_select_db($database_connect, $connect);
$query_thesisEdit = sprintf("SELECT * FROM thesis WHERE thesisID = %s AND thesisTeacherID = %s", $colname_thesisEdit, $colname2_thesisEdit);
$thesisEdit = mysql_query($query_thesisEdit, $connect) or die(mysql_error());
$row_thesisEdit = mysql_fetch_assoc($thesisEdit);
$totalRows_thesisEdit = mysql_num_rows($thesisEdit);
//-----------------------------------------------thesisEdit Sorgusu
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE thesis SET  thesisTopic=%s, thesisTeacherID=%s, thesisCriteria=%s, thesisDivisionID=%s WHERE thesisID=%s",
                       GetSQLValueString($_POST['thesisTopic'], "text"),
                       GetSQLValueString($_POST['thesisTeacherID'], "int"),
                       GetSQLValueString($_POST['thesisCriteria'], "text"),
                       GetSQLValueString($_POST['thesisDivisionID'], "int"),
                       GetSQLValueString($_POST['thesisID'], "int"));

  mysql_select_db($database_connect, $connect);
  if( (SAFE_POST('password') == $row_loggeduser['teacherPassword'])   &&  ($row_thesisEdit['thesisTeacherID']==$row_loggeduser['teacherID'])  ){
  	// Üst ifade dışarıdan girişleri engellemek ve şifre kontrolünü sağlamak için
  	$Result1 = mysql_query($updateSQL, $connect) or die(mysql_error());
	// Güncellemeyi anında göstermek için ob_Start ve ob_end_flush ile headerları serbest bırakıp, ardından
	// sayfayı reload ediyoruz..
	header( "refresh:0; url=teacher_edit_thesis.php?thesisID=".SAFE_GET('thesisID') );
  }
...
<div id="container">
		...
		<div id="container_col_middle">
		  <div class="NewsFeed">
		  	<div class="NFright">
			  <div class="introDisplayer">TEZ KONUSUNU DÜZENLE </div>
			  <div id="statusMessage"><?php if($insertStatus==1){?>Konu Ekleme Başarılı<? } ?><?php if($insertStatus==2){?>Şifreniz Yanlış<? } ?></div>
		  	  <form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST" onsubmit="return formValidate()">
		  	    <table width="497" height="274" border="0" cellpadding="0" cellspacing="4">
                  <tr>
                  ...
                  <tr>
                    ...
                    <td><input name="thesisTopic" type="text" class="formTextfield2" id="thesisTopic" value="<?php echo $row_thesisEdit['thesisTopic']; ?>" />
                        <div class="validationInfo"></div></td>
                  </tr>
                  <tr>
                    <td height="113" valign="top"><span class="style23">Başvuru Kriterleriniz </span></td>
                    <td valign="top"><p>
                        <textarea name="thesisCriteria" cols="55" rows="15" class="formTextfield2" id="thesisCriteria" style="height:70px;"><?php echo $row_thesisEdit['thesisCriteria']; ?></textarea>
<div class="validationInfo"></div>
				  ...
				  <tr>
                    ...
                    <td><?php do {?>
                        <input name="thesisDivisionID" type="radio" 
						<?php if($row_divisions2['divisionID']==$row_thesisEdit['thesisDivisionID']){ ?> checked="checked" <?php } ?>
						value="<?=$row_divisions2['divisionID']?>" /><?=$row_divisions2['divisionTitle']?> <br />
						 <?php } while ($row_divisions2 = mysql_fetch_assoc($divisions2)); ?>
						<div class="validationInfo"></div></td>
                  </tr>
                  <tr>
                    <td height="33"><input name="thesisTeacherID" type="hidden" id="thesisTeacherID" value="<?php echo $row_loggeduser['teacherID'];?>" />
                    <input name="thesisID" type="hidden" id="thesisID" value="<?php echo $row_thesisEdit['thesisID']; ?>" />
                    <input name="validate" type="hidden" id="validate" value="yes" /></td>
                    <td><input type="submit" id="formGonder" class="buttonStyle buttonStyleSubmit" name="Submit" value="Konuyu Güncelle" /></td>
                  </tr>
                </table>
