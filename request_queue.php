...
$query_thesis1 = sprintf("SELECT * FROM requests WHERE requestStudentID='%d' ORDER BY requestID ASC", $loggedUserID);
...
<?php 
// İstek / Request Silme---------------------------------------------------
$requestToBeDeleted = SAFE_GET('requestID');
$delete = SAFE_GET('delete');
// Güvenlik önlemi
if( ($delete==1) && (!is_numeric($requestToBeDeleted))){ exit(0); }
// Silinecek request in user kontrolünü yapalım.
mysql_select_db($database_connect, $connect);
$query_deleterUser = sprintf("SELECT * FROM requests WHERE requestID='%d' AND requestStudentID='%d'", $requestToBeDeleted, $row_loggeduser['userID']);
...
if(($delete == 1) && ($totalRows_deleterUser==1) ){
	$toBeDeleted = sprintf("DELETE FROM requests WHERE requestID='%d' AND requestStudentID='%d'", $requestToBeDeleted, $row_loggeduser['userID']);
	if( mysql_query($toBeDeleted) ){
		// Eğer silme işlemi başarılıysa sayfayı refresh edelim.
		header( "refresh:0; url=request_queue.php" );
	}
}
// --------------------------------------------------------İstek /Request Silme Bitiş
?>
<!-- 
/************************
 * TezSis: TezYonetimSistemi
...
<div id="container">
		...
		<div id="container_col_middle">
		  <div class="NewsFeed">
		  	<div class="NFright">
			  <div class="introDisplayer">BEKLEYEN KONU İSTEKLERİNİZ </div>
		      ...
		      <div class="newsTable">
		        <table class="displayerTable" width="617" border="0" cellspacing="1" cellpadding="4">
				  ...
				  // Kabul edilen istek ---------------------------------------------------
				    mysql_select_db($database_connect, $connect);
					$query_acceptedRequest = sprintf("SELECT * FROM accepted_requests WHERE arequestStudentID='%d'",$row_loggeduser['userID']);
				  ...
				  ?>
                  <?php if($totalRows_requestedThesis==1){$bgColor = '#E2F9C1'; ?>
                  <tr style="background-color:<?=$bgColor?>;">
                    <td height="38">&nbsp;</td>
                    <td><strong>Kabul Edildi</strong> </td>
				 ...
				 <?php
				  // Reddedilen istekler ------------------------------------------------------
				    mysql_select_db($database_connect, $connect);
					$query_refusedRequest = sprintf("SELECT * FROM refused_requests WHERE rrequestStudentID='%d'",$row_loggeduser['userID']);
					...
					$totalRows_requestedThesis = mysql_num_rows($requestedThesis);
				  ?>
                  <?php if($totalRows_requestedThesis>=1){$bgColor = '#FFD5D5'; ?>
                  <tr style="background-color:<?=$bgColor?>;">
                    <td height="38">&nbsp;</td>
                    <td><strong>Reddedildi</strong> </td>
                    <td><?php echo $row_requestedThesis['thesisTopic']; ?><br />
						<div id="thesisCriteria"><?php echo $row_requestedThesis['thesisCriteria']; ?></div></td>
                    <td><?php
					mysql_select_db($database_connect, $connect);
                 ...
                 <?php  //--------------------------------------------reddedilen istekler  ?>
                  ...
                  <tr style="background-color:<?=$bgColor?>;">
                    ...
                    <td><strong>Cevap Bekleniyor</strong><br />
                      <a href="request_queue.php?delete=1&requestID=<?=$row_thesis1['requestID']?>">İptal Et</a> </td>
                    <td><?php echo $row_requestedThesis['thesisTopic']; ?><br />
						<div id="thesisCriteria"><?php echo $row_requestedThesis['thesisCriteria']; ?></div></td>
                    <td><?php
					mysql_select_db($database_connect, $connect);
